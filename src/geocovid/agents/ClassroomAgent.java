...
public class ClassroomAgent extends WorkplaceAgent {
	...
	public ClassroomAgent(SubContext subContext, int sectoralType, int sectoralIndex, Coordinate coord, long id, String workType, int activityType, int area, int coveredArea, int workersPlace) {
		super(subContext,  sectoralType,  sectoralIndex,  coord,  id,  workType,  activityType,  area, coveredArea, workersPlace);
			...
			if (col >= x) {
				...
				if (row >= y) {
					if (i+1 < studentPositionsCount) { //TODO esto lo deje como info por las dudas
						...
						if (fullBuilding) {
							System.out.format("Cupos de estudiantes limitados a %d de %d en tipo: %s id: %d%n",
							i+1, studentPositionsCount, workplaceType, getId());
